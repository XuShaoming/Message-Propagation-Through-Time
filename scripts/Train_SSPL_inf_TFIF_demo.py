...
x_train_sp_cur = x_train_sp.clone().detach()
for epoch in range(epochs):
    # Set the model in training mode.
    starttime=time.time()
    if epoch < decay_epochs:
        decay = inverse_sigmoid_decay(epoch, epochs, alpha, beta)
    else:
        decay = 0
    model1.train()
    if epoch > 0:
        x_train_sp_cur = x_train_sp_next
    x_train_sp_next = x_train_sp.clone().detach()
    ...
    for j in range(0,num_train_samples,batch_size):
        ...
        batch_x = x_train_sp_cur[idx[sj:ej],:,:]
        ...
        hiddens.detach_()
        pred.detach_()
        schedule_mask = torch.bernoulli(torch.ones(pred.size()) * decay).to(device)
        # Note: need to save the shifted predictions.         
        x_train_sp_next[idx[sj:ej],1:,-1] = x_train_sp_next[idx[sj:ej],1:,-1] * schedule_mask[:,:-1,-1] + pred[:,:-1,-1] * torch.logical_not(schedule_mask[:,:-1,-1])
