...
notes = [
    'Goal: Simulate the target variable using 7 weather drivers including Day of Year, PRECIPmm, TMP_MXdgC, TMP_MNdgC, SOLARMJ/m2, WINDM/s, RHmd.'
    'Target variables: SW_ENDmm, SNOmm, or Q_pred_mm.'
...
epoch_warmup=int(sys.argv[15])
stride=int(sys.argv[16])
keeper=int(sys.argv[17])
...
## Set stride
num_samples_train = 0
shift_train = int(stride)
num_samples_valid = 0
shift_valid = int(stride)
num_samples_test = 0
shift_test = int(stride)
...
def get_key_map(data_index, basin_index, window):
    key_map = dict()
    heads = data_index[:,0]
    for head in heads:
        key_map[(basin_index[0], head)] = heads[np.logical_and(heads>head, heads <= head+window)]
    
    return key_map
# Get train_key_map

basin_index = np.array([1])
train_key_map = get_key_map(train_idx_arr, basin_index, n_steps)

# Initialize State Map
train_state_map = dict()
for key in train_key_map.keys():
    h0 = model1.init_hidden(1)
    h0 = h0.to(device)
    # [mu, h, c]
    train_state_map[key] = [h0, h0, 0]
...
for epoch in range(epochs):
    ...
    starttime=time.time()
    model1.train()
    idx=torch.randperm(x_train_sp.size()[0])
    for j in range(0,num_train_samples,batch_size):
        ...
        batch_idx_arr = train_idx_arr[idx[sj:ej],:]
        hidden_head = model1.init_hidden(ej - sj)
        if len(batch_idx_arr.shape) == 1:
            batch_idx_arr = batch_idx_arr[None,:]
        
        # Pass messages after the warmup epoch. 
        if epoch > epoch_warmup:  
            # Fetch policy
            head_keys = batch_idx_arr[:,0]
            for i, head_key in enumerate(head_keys):
                mu0, h0, c = train_state_map[(basin_index[0], head_key)]
                if c == 0 :
                    message = mu0
                elif c > 0:
                    message = (keeper * mu0 + c * h0) / (keeper + c)
                else:
                    raise RuntimeError("count < 0")
                hidden_head[0,i,:] = message[0,0,:]
        hidden_head = hidden_head.to(device)
        ...
        # Update policy
        hiddens.detach_()
        base_keys =  batch_idx_arr[:,0]
        for i, base_key in enumerate(base_keys):
            head_keys = train_key_map[(basin_index[0],base_key)]
            for head_key in head_keys:
                mu0, h0, c = train_state_map[(basin_index[0], head_key)]
                # h0 = (c * h0 + hiddens[i][head_key-base_key-1].reshape(1,1,-1)) / (c + 1)
                h0 = c / (c+1) * h0 + 1 / (c+1) * hiddens[i][head_key-base_key-1].reshape(1,1,-1)
                train_state_map[(basin_index[0], head_key)] = [mu0, h0, c+1]
    ...
    # Pass policy
    for key in train_state_map.keys():
        mu0, h0, c = train_state_map[key]
        if c > 1:
            mu0 = keeper / (keeper + c) * mu0 + c / (keeper + c) * h0
            h0 = model1.init_hidden(1)
            train_state_map[key] = [mu0, h0, 0]  
...
np.save(exp_dir+'run_iter_{}_epoch_loss_train_arr'.format(run_iter),epoch_loss_train_arr)
np.save(exp_dir+'run_iter_{}_epoch_loss_valid_arr'.format(run_iter),epoch_loss_valid_arr)
np.save(exp_dir+'run_iter_{}_epoch_loss_test_arr'.format(run_iter),epoch_loss_test_arr)
