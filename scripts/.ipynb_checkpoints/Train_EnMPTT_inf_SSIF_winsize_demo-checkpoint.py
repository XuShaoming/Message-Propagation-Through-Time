...
eraser=int(sys.argv[17])
train_years=int(sys.argv[18])
stride=int(n_steps/2)
...
train_x = train_x[-train_years*366:].copy()
train_y = train_y[-train_years*366:].copy()
...
for epoch in range(epochs):
    ...
    if epoch_warmup > 0 and epoch == epoch_warmup + 1:
        model1.eval()
        with torch.no_grad():
            # Apply SSIF in Linkage reconstruction
            pred_train,hiddens = model1(x_train_,model1.init_hidden(1))
            hiddens.detach_()
            keys = [key[1] for key in train_state_map.keys()]
            for key in keys:
                mu0, h0, c = train_state_map[(basin_index[0], key)]
                mu0 = hiddens[0][key].reshape(1,1,-1)
                train_state_map[(basin_index[0], key)] = [mu0, h0, c]  
    model1.train()    
        ...
        if epoch > epoch_warmup:  
            ...
            for i, head_key in enumerate(head_keys):
                ...
                if c == 0 :
                    ...
                    message = (eraser * mu0 + c * h0) / (eraser + c)
                else:
    ...
    for key in train_state_map.keys():
        ...
        if c > 1:
            mu0 = eraser / (eraser + c) * mu0 + c / (eraser + c) * h0
            h0 = model1.init_hidden(1)
