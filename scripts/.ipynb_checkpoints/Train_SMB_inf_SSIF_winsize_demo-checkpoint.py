...
if num_train_samples <= batch_size:
    for epoch in range(epochs):
        starttime=time.time()
        # Set the model in training mode. This is important if you apply dropout or batch_norm
        # in network. You should trun on model1.eval() in validation and testing.
        model1.train()
        # Shuffle the data before trainig. The variability introduced in this step can help to avoid overfitting and train 
        # robust model.
        ...
        for j in range(0,num_train_samples,batch_size):
            # Sets the gradients of all optimized torch.Tensors to zero. If this is missing, then the gradient will accumulate
            ...
            optimizer.step()
            # with torch.no_grad():
            #     # Save losses on each classes
            #     epoch_losses[epoch] = epoch_losses[epoch] + batch_losses.cpu().numpy() * (torch.sum(batch_m).cpu().numpy() + 1)
            #     # Save total loss.
            #     epoch_loss[epoch] = epoch_loss[epoch] + batch_loss.cpu().numpy() * (torch.sum(batch_m).cpu().numpy() + 1)
        # Call the learning rate scheduler on epoch level.
        scheduler.step()
        endtime=time.time()
        train_time = train_time + endtime - starttime

        # In validation, turn on eval() mode which will disable dropout and batchNorm.
        model1.eval()
        with torch.no_grad():
            ...
            '''
            Save the model as soon as both MSE and R2 are improved on validataion set.
            '''
            ...
            # Please note, the losses are computed on z-norm target variables.
